...
**The pitch inset in a clip** — An exported clip can carry the pitch in its **bottom-left corner**, sized as a share of the frame so a 720p clip and a 1080p one look the same. It is **top-down**: the diagram is drawn by the export path's own canvas renderer from the same model the two on-screen surfaces read, so no GPU is involved and nothing about a WebGL context can change a file. It shows the moment's positions with **shape as well as colour** — a disc for the first team, a square for the second — and a legend naming both, because a shirt number is illegible at that size and colour alone is never enough (NFR-34). The moment's tag and timecode sit beside the diagram, and the panel says **how many of the selected moments have positions** before the export, so a clip without an inset is never a surprise.
